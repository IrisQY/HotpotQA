...
#        ques_idxs = torch.LongTensor(self.bsz, self.ques_limit, 50).cuda()
        ...
        # para_len_tensor = torch.Tensor(self.bsz, 10)
        para_mapping = torch.Tensor(self.bsz, self.para_limit, 10).cuda()
        ...
        while True:
            ...
            for i in range(len(cur_batch)):
                ...
                # para_len_tensor[i].copy_(cur_batch[i]['para_sent_lens'])
                if cur_batch[i]['y1'] >= 0:
                    y1[i] = cur_batch[i]['y1']
                    y2[i] = cur_batch[i]['y2']
                    q_type[i] = 0
                ...
                para_so_far = 0
                for k_th_para in range(len(cur_batch[i]['para_sent_lens'])): # len()
                    k_th_para_len = cur_batch[i]['para_sent_lens'][k_th_para]

                    para_mapping[i,:,k_th_para] = torch.sum(all_mapping[i, :, para_so_far:para_so_far + k_th_para_len], dim = -1).unsqueeze(2)
                    para_so_far = para_so_far + k_th_para_len
...
def evaluate_classifier(eval_file, answer_dict, classifier_accuracy, step=0, config=None, tbx=None, pred_dict=None, eval_path=None, split=None, num_visuals=None):
    f1 = exact_match = total = 0
    ids = []
    for key, value in answer_dict.items():
        total += 1
    ...
    return {'exact_match': classifier_accuracy, 'f1': f1}

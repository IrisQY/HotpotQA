...
class SPModel(nn.Module):
    def __init__(self, config, word_mat, char_mat):
        ...
        self.cache_S = 0
        self.elmoProj = nn.Sequential(nn.Linear(2 * self.word_dim, self.word_dim, bias = False), nn.Linear(self.word_dim, self.word_dim, bias = False))
    ...
    def forward(self, context_idxs, ques_idxs, context_char_idxs, ques_char_idxs, context_lens, start_mapping, end_mapping, all_mapping, return_yp=False):
        ...
        with torch.no_grad():
            ...
            context_word = torch.cat((context_word[0], context_word[1]), -1)
#        print('The shape of context_word is', context_word.size())
        context_word = self.elmoProj(context_word)
        # ques_word = self.word_emb(ques_idxs)
#        print('The shape of context_word after projection', context_word.size())
        with torch.no_grad():
            ques_word =  elmo(ques_idxs)['elmo_representations']
            ques_word = torch.cat((ques_word[0], ques_word[1]), -1)
#        print('The shape of ques_word', ques_word.size())
        ques_word = self.elmoProj(ques_word)
#        print('The shape of ques_word after projection', ques_word.size())

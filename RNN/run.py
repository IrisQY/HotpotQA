    ...
    for epoch in range(10):####
        for num_of_buckets in range(total_num_of_buckets):
            ...
            for data in build_train_iterator(num_of_buckets):
                ...
                ###
                # para_mapping = Variable(data['para_mapping'])
                ###
                ...
                # logit1, logit2, predict_type, predict_support = model(context_idxs, ques_idxs, context_char_idxs, ques_char_idxs, context_lens, start_mapping, end_mapping, all_mapping, para_mapping, return_yp=False)
                ...
                if global_step % config.checkpoint == 0:
                    model.eval()
                    metrics = evaluate_batch(build_dev_iterator(), model, 0, dev_eval_file, config, global_step, tbx)
                    ###
...
def evaluate_batch(data_source, model, max_batches, eval_file, config, step=0, tbx=None):
    ...
    for step, data in enumerate(iter):
        ...
        # logit1, logit2, predict_type, predict_support, yp1, yp2 = model(context_idxs, ques_idxs, context_char_idxs, ques_char_idxs, context_lens, start_mapping, end_mapping, all_mapping, para_mapping, return_yp=True)
